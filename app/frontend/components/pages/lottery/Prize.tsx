...
export default function LotterySection({ eventId }: { eventId: string }) {
  ...
  // On aplatit tous les prizes de toutes les lotteries en un seul tableau
              ...
              <div className="relative h-56 w-full rounded-2xl overflow-hidden shadow-lg border-4 border-white transform group-hover:scale-105 transition-transform duration-500 bg-surface">
                {prize.imageUrl ? (
                  <Image
                    src={prize.imageUrl}
                    fill
                    className={`object-cover ${index === 0 ? 'animate-spin-slow reverse' : ''}`}
                    alt={prize.title}
                  />
                ) : (
                  <div className="flex items-center justify-center h-full bg-muted/10">
                    <Trophy
                      className={`text-muted/20 ${index === 0 ? 'animate-spin-slow ' : ''}`}
                      size={48}
                    />
                  </div>
                )}
